#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""Module for analyzing direct beam images.

This module provides the beam_analysis class for calculating center of mass,
statistical information, and Gaussian fits from direct beam images in EDF or
numpy data format.
"""
...
class BeamAnalysis():
    """Basic methods to analyze direct beam images.
    ...
    def __init__(self, beamdata, beamheader=None, boxsize=50):
        """Initialize the BeamAnalysis class."""
        ...
        hbs = 0.5 * self.boxsize
        ax, bx  = int(self.Cx - hbs), int(self.Cx + hbs)
        ay, by  = int(self.Cy - hbs), int(self.Cy + hbs)
    ...
    def vector_mass_center(self, data):
        """Center of mass of a given 1D numpy array "data"."""
        # Total mass.
        mass = np.sum(data)
        # Center of mass.
    ...
    def matrix_center(self, data):
        """Center of mass of a given 2D numpy array "data"."""
        cx, cy, mass = 0.0, 0.0, 0.0
        nlin = data[:, 0].size   # Number of lines of data.
        ncol = data[0].size      # Number of columns of data.
    ...
    def profile_cut(self):
        """Beam profile in x and y direction.

        Passing through the calculated center of the beam.
        """
        rbs = 0.5 * self.boxsize    # Half-box size.
        # Vertical and horizontal limits.
        ax, bx = int(self.Cx - rbs), int(self.Cx + rbs)
        ay, by = int(self.Cy - rbs), int(self.Cy + rbs)
    ...
    def skewness(self, profile):
        """Calculate the skewness of the given histogram.

        This function considers the 'profile' of the beam, in the vertical or
    ...
    def flattened_profile(self):
        """Projection of the 2D histogram onto x and y directions."""
        nx, ny = self.data.shape
        self.flatx = np.array(
            [np.sum(self.data[:, jj]) for jj in range(ny)]) / nx
        self.flaty = np.array(
            [np.sum(self.data[jj, :]) for jj in range(nx)]) / ny
        return
